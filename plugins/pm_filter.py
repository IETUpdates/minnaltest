...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("ഇത് നിനക്ക് ഉള്ളതല്ല. നിനക്ക് വേണമെങ്കിൽ നീ request ചെയ്യ്", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
    ...
    if query.data == "close_data":
        ...
        if chat_type == "private":
            ...
            if (st.status == "creator") or (str(userid) in ADMINS):
                ...
                await query.answer("ഇത് നിനക്ക് ഉള്ളതല്ല. നിനക്ക് വേണമെങ്കിൽ നീ request ചെയ്യ് 🤓!!",show_alert=True)
    ...
    if query.data.startswith("file"):
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                await query.answer(' ഞാൻ പേഴ്സണൽ ആയി അയച്ചിട്ടുണ്ട് പോയി നോക്ക് Check PM, I have sent files in pm',show_alert = True)
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("JOIN ചെയ്യാൻ പറഞ്ഞാൽ അത് ചെയ്യ് .I Like Your Smartness, But Don't Be Oversmart 😒",show_alert=True)
            return
        ident, file_id = query.data.split("#")
        files_ = await get_file_details(file_id)
        if not files_:
        ...
        buttons = [[
            ...
            InlineKeyboardButton('👮 My Owner', url='https://t.me/iqbal_ka'),
